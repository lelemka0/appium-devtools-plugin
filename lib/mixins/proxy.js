...
/**
 * checks if the code number is allowed by `WebSocket.close`.
 * @param {number} code 
 * @returns {boolean}
 */
...
/**
 * @this {import('../plugin').DevtoolsPlugin}
 * @param {string} forwardToUrlPattern
 * @param {string} entityIdPlaceholder
 * @returns {WebSocket.Server}
 */
function prepareWebSocketForwarder (forwardToUrlPattern, entityIdPlaceholder) {
  ...
  wss.on('connection', (
    ...
    const accumulatedMsg = [];
    const accumulatedEmitter = setInterval(() => {
      if (wsDownstream.readyState === WebSocket.OPEN) {
        let msgObj;
        while (msgObj = accumulatedMsg.shift()) {
          wsDownstream.send(msgObj.msg, {binary: msgObj.binary});
        }
      }
    }, 5e3);
    ...
    wsUpstream.on('message', (msg, binary) => {
      if (wsDownstream.readyState === WebSocket.OPEN) {
        ...
        accumulatedMsg.push({msg, binary});
      }
    });
    ...
    wsDownstream.once('close', (code, reason) => {
      clearInterval(accumulatedEmitter);
      this.logger.info(
        `The downstream ${dstUrl} has been closed: ${code}, ` +
        `${reason || '(no reason given)'}`
      );
      if (wsUpstream.readyState === WebSocket.OPEN) {
        wsUpstream.close(isValidErrorCode(code) ? code : WS_SERVER_ERROR, reason);
      }
    });
